    ...
    for j in range(len(sequence_of_sequences)):
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j][k]) is int:
                retlist.append(sequence_of_sequences[j][k])
...
def run_test_big_letters():
    ...
    # Test 2:
    expected = 'OTSS'
    answer = big_letters(['Ok what is ThiSS?',  # OTSSS
                          [],  # not a string
                          ['oops'],  # not a string
                          'oops',
                          'oops',
...
def big_letters(sequence_of_sequences):
    ...
    for j in range(len(sequence_of_sequences)):
        for k in range(len(sequence_of_sequences[j])):
            if type(sequence_of_sequences[j]) is str and \
                    sequence_of_sequences[j][k].isupper():
                retstring = retstring + sequence_of_sequences[j][k]
    return retstring
